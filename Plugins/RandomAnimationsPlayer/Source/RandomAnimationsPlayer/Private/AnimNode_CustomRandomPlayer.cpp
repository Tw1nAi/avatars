...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimNode_CustomRandomPlayer)

FAnimNode_CustomRandomPlayer::FAnimNode_CustomRandomPlayer() : CurrentPlayDataIndex(0), bShuffleMode(false) {}

void FAnimNode_CustomRandomPlayer::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
  ...
  {
    FRandomPlayerSequenceEntryCustom* Entry = &Entries[EntryIndex];
  ...
  for (FRandomPlayerSequenceEntryCustom* Entry : ValidEntries)
  {
  ...
  // blend between them.
  FRandomAnimPlayDataCustom& CurrentData = GetPlayData(ERandomDataIndexTypeCustom::Current);
  InitPlayData(CurrentData, CurrentEntry, 1.0f);

  FRandomAnimPlayDataCustom& NextData = GetPlayData(ERandomDataIndexTypeCustom::Next);
  InitPlayData(NextData, NextEntry, 0.0f);
}

void FAnimNode_CustomRandomPlayer::Update_AnyThread(const FAnimationUpdateContext& Context)
{
  ...
  const TArray<FRandomPlayerSequenceEntryCustom>* InputEntries = GetEvaluateGraphExposedInputs().Get<TArray<FRandomPlayerSequenceEntryCustom>>(FName(TEXT("Entries")));
  ...
  if (CurrentData->RemainingLoops <= 0)
  {
    ...
    FRandomPlayerSequenceEntryCustom& NextSequenceEntry = *NextData->Entry;
  ...
  if (bAdvanceToNextEntry)
  {
    ...
    CurrentData = &GetPlayData(ERandomDataIndexTypeCustom::Current);
    NextData = &GetPlayData(ERandomDataIndexTypeCustom::Next);
  }

  FAnimTickRecord TickRecord(
      CurrentData->Entry->Sequence, true, CurrentData->PlayRate, false, CurrentData->BlendWeight, CurrentData->CurrentPlayTime, CurrentData->MarkerTickRecord
  );
...
void FAnimNode_CustomRandomPlayer::Evaluate_AnyThread(FPoseContext& Output)
{
  ...
  if (!FMath::IsNearlyEqualByULP(CurrentData.BlendWeight, 1.0f))
  {
    ...
    CurrentSequence->GetAnimationPose(
        CurrentPoseData,
        FAnimExtractContext(static_cast<double>(CurrentData.CurrentPlayTime), AnimProxy->ShouldExtractRootMotion(), CurrentData.DeltaTimeRecord, CurrentData.RemainingLoops > 0)
    );
    NextSequence->GetAnimationPose(
        NextPoseData,
        FAnimExtractContext(static_cast<double>(NextData.CurrentPlayTime), AnimProxy->ShouldExtractRootMotion(), NextData.DeltaTimeRecord, NextData.RemainingLoops > 0)
    );
  ...
  else
  {
    ...
    CurrentSequence->GetAnimationPose(
        AnimationPoseData,
        FAnimExtractContext(
            static_cast<double>(CurrentData.CurrentPlayTime), Output.AnimInstanceProxy->ShouldExtractRootMotion(), CurrentData.DeltaTimeRecord, CurrentData.RemainingLoops > 0
        )
    );
  }
}

void FAnimNode_CustomRandomPlayer::GatherDebugData(FNodeDebugData& DebugData)
{
...
UAnimationAsset* FAnimNode_CustomRandomPlayer::GetAnimAsset() const
{
  UAnimationAsset* AnimationAsset = nullptr;

  if (ValidEntries.Num() > 0)
  {
    const FRandomAnimPlayDataCustom& CurrentPlayData = GetPlayData(ERandomDataIndexTypeCustom::Current);
...
float FAnimNode_CustomRandomPlayer::GetAccumulatedTime() const
{
...
bool FAnimNode_CustomRandomPlayer::GetIgnoreForRelevancyTest() const
{
  return GET_ANIM_NODE_DATA(bool, bIgnoreForRelevancyTest);
}

bool FAnimNode_CustomRandomPlayer::SetIgnoreForRelevancyTest(bool bInIgnoreForRelevancyTest)
{
...
float FAnimNode_CustomRandomPlayer::GetCachedBlendWeight() const
{
  return BlendWeight;
}

void FAnimNode_CustomRandomPlayer::ClearCachedBlendWeight()
{
  BlendWeight = 0.f;
}

const FDeltaTimeRecord* FAnimNode_CustomRandomPlayer::GetDeltaTimeRecord() const
{
  if (ValidEntries.Num() > 0)
  {
    const FRandomAnimPlayDataCustom& CurrentPlayData = GetPlayData(ERandomDataIndexTypeCustom::Current);
...
int32 FAnimNode_CustomRandomPlayer::GetNextValidEntryIndex()
{
...
FRandomAnimPlayDataCustom& FAnimNode_CustomRandomPlayer::GetPlayData(ERandomDataIndexTypeCustom Type)
{
  ...
  if (Type == ERandomDataIndexTypeCustom::Current)
  {
...
const FRandomAnimPlayDataCustom& FAnimNode_CustomRandomPlayer::GetPlayData(ERandomDataIndexTypeCustom Type) const
{
  return const_cast<FAnimNode_CustomRandomPlayer*>(this)->GetPlayData(Type);
}

void FAnimNode_CustomRandomPlayer::InitPlayData(FRandomAnimPlayDataCustom& Data, int32 InValidEntryIndex, float InBlendWeight)
{
  FRandomPlayerSequenceEntryCustom* Entry = ValidEntries[InValidEntryIndex];
...
void FAnimNode_CustomRandomPlayer::AdvanceToNextSequence()
{
...
void FAnimNode_CustomRandomPlayer::BuildShuffleList(int32 LastEntry)
{

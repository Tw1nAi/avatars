...
#include "AnimNode_CustomRandomPlayer.generated.h"

enum class ERandomDataIndexTypeCustom
{
  Current,
  Next,
};
...
struct RANDOMSEQUENCEPLAYER_API FRandomPlayerSequenceEntryCustom
{
  GENERATED_BODY()

  FRandomPlayerSequenceEntryCustom() : Sequence(nullptr), ChanceToPlay(1.0f), MinLoopCount(0), MaxLoopCount(0), MinPlayRate(1.0f), MaxPlayRate(1.0f) {}
...
struct RANDOMSEQUENCEPLAYER_API FRandomAnimPlayDataCustom
{
  // Index into the real sequence entry list, not the valid entry list.
  FRandomPlayerSequenceEntryCustom* Entry = nullptr;
...
struct RANDOMSEQUENCEPLAYER_API FAnimNode_CustomRandomPlayer : public FAnimNode_AssetPlayerRelevancyBase
{
  GENERATED_BODY()

  FAnimNode_CustomRandomPlayer();

public:
  /** List of sequences to randomly step through */
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Settings", meta = (PinShownByDefault))
  TArray<FRandomPlayerSequenceEntryCustom> Entries;
...
private:
  // Return the index of the next FRandomPlayerSequenceEntryCustom to play, from the list
  ...
  FRandomAnimPlayDataCustom& GetPlayData(ERandomDataIndexTypeCustom Type);
  const FRandomAnimPlayDataCustom& GetPlayData(ERandomDataIndexTypeCustom Type) const;
  ...
  void InitPlayData(FRandomAnimPlayDataCustom& Data, int32 InValidEntryIndex, float InBlendWeight);
  ...
  TArray<FRandomPlayerSequenceEntryCustom*> ValidEntries;
  ...
  TArray<FRandomAnimPlayDataCustom> PlayData;

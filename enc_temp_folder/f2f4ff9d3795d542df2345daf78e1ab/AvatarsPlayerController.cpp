...
void AAvatarsPlayerController::BeginPlay()
{
  ...
  if (UPersistanceController* Persistance = UAvatarsGame::GetPersistance(GetWorld()))
  {
    ...
    ULog::Info("Applied loaded setting: ChangeAvatarTimeout: " + FString::SanitizeFloat(ChangeAvatarTimeout.TimeoutDelay));
    ...
    ULog::Info("Applied loaded setting: IdleGreetingTimeout: " + FString::SanitizeFloat(IdleGreetingTimeout.TimeoutDelay));

    bUseLocalTranscription = Persistance->ConversationSettings.TranscriptionApi == FConversationSettings::RestApiName;
    ULog::Info("Applied loaded setting: TranscriptionApi: " + Persistance->ConversationSettings.TranscriptionApi);

    UOpenAIUtils::setOpenAIApiKey(Persistance->ConversationSettings.GetOpenApiKey());
    ULog::Info("Applied loaded setting: OpenAI Api Key: " + Persistance->ConversationSettings.GetOpenApiKey());
  }
...
void AAvatarsPlayerController::StartAudioCaptureTimer()
{
  bAudioCaptureBelowMinDuration = false;
  AudioCaptureTimer = GetWorld()->GetTimeSeconds();
}

bool AAvatarsPlayerController::CheckAudioCaptureDuration()
{
  ...
  return bAudioCaptureBelowMinDuration;
}

void AAvatarsPlayerController::OnAudioCaptureTimerBelowMinimum()
{
  bAudioCaptureBelowMinDuration = false;
  const FString InfoMessage = "Audio capture duration is below the minimum duration.";
  UE_LOG(LogAwatarsPlayerController, Display, TEXT("%s"), *InfoMessage);

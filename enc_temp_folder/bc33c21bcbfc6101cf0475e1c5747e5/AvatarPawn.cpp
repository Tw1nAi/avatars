...
void AAvatarPawn::SetState(EAvatarState NewState)
{
  ...
  if (BodyAnimBP == nullptr)
  {
    if (bDebug)
    {
      FString Message = FString::Printf(TEXT("Could not get reference to UBodyAnimInstance* BodyAnimBP while setting avatar state: %s"), *MapStateToString(NewState));
      ULog::Error(Message);
    }
    return;
  }
...
bool AAvatarPawn::GetDialogSoundWave(const FString Path, const FString AssetName, FString LanguageString, USoundWave*& OutDialogSound)
{
  ...
  {
    if (bDebug) ULog::Error("Could not get reference to USoundWave: " + AssetName + " for path: " + AudioPath + "in object: " + this->GetName());
...
bool AAvatarPawn::GetDialogFaceAnimation(const FString Path, const FString AssetName, FString LanguageString, UAnimSequence*& OutFaceAnimation)
{
  ...
  {
    if (bDebug) ULog::Error("Could not get reference to UAnimSequence: " + FullName + " for path: " + AnimationPath + "in object: " + this->GetName());
    return false;
  }
...
void AAvatarPawn::PlayDialogFaceAnimation(UAnimSequence* FaceAnimation)
{
  ...
  if (FaceAnimBP == nullptr)
  {
    if (bDebug)
    {
      FString Message = FString::Printf(TEXT("Could not get reference to UFaceAnimInstance* FaceAnimBP while playing dialog animation: %s"), *FaceAnimation->GetName());
      ULog::Error(Message);
    }
    return;
  };
